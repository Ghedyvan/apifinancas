...
class InfoMoneyScrapperService {
  constructor() {
    this.baseUrl =
      "https://api.infomoney.com.br/ativos/top-alta-baixa-por-ativo/acao";
    this.params = {
      sector: "Todos",
  ...
  async saveToSupabase(stockData) {
    try {
      console.log(
        `💾 Substituindo ${stockData.length} registros de ações no Supabase...`
      );

      // Verificar se as credenciais estão carregadas
      console.log(`🔐 Supabase URL: ${this.supabaseUrl ? "OK" : "MISSING"}`);
      console.log(`🔑 Supabase Key: ${this.supabaseKey ? "OK" : "MISSING"}`);

      // Verificar conexão com Supabase
      const { count: testCount, error: testError } = await this.supabase
        .from("stock_data")
      ...
      console.log(`📋 Total de registros existentes: ${testCount || 0}`);

      // Extrair todos os stock_codes para deletar
      const stockCodes = stockData.map((item) => item.stock_code);
      console.log(
        `📝 Stock codes a processar: ${stockCodes.slice(0, 5).join(", ")}${
          stockCodes.length > 5 ? "..." : ""
        }`
      );

      console.log(
        `🗑️ Deletando registros antigos de ${stockCodes.length} ações...`
      );

      // 1. Deletar todos os registros existentes destes ativos em lote
      ...
      console.log(
        `✅ Registros antigos removidos para ${stockCodes.length} ações`
      );

      // Verificar estrutura dos dados antes de inserir
      console.log(
        `📊 Exemplo de dado a inserir:`,
        JSON.stringify(stockData[0], null, 2)
      );

      console.log(`📥 Inserindo ${stockData.length} novos registros...`);

      // 2. Inserir todos os novos registros em lotes menores
      const batchSize = 50; // Reduzir para identificar problemas
      let totalInserted = 0;
      let errorCount = 0;
      ...
      console.log(
        `📊 Resumo Final: ${totalInserted} ações inseridas, ${errorCount} erros`
      );
      ...
      if (countError) {
        console.error(
          "❌ Erro ao contar registros finais:",
          countError.message
        );
      } else {
        console.log(
          `📈 Total de registros únicos no banco: ${finalCount || 0}`
        );
      }

      // Verificar se todas as ações foram inseridas corretamente
      if (finalCount && finalCount >= stockData.length) {
        console.log(
          `🎯 Sucesso! Todas as ${stockData.length} ações foram atualizadas`
        );
      } else {
        console.log(
          `⚠️ Atenção: Esperado ${stockData.length}, encontrado ${
            finalCount || 0
          }`
        );
      }
  ...
  async getAllPages() {
    const brasiliaTime = this.getBrasiliaTime();
    console.log(
      `🚀 Iniciando coleta de dados - ${brasiliaTime.toLocaleString("pt-BR", {
    ...
    try {
      ...
      if (!this.hasRunInitially || this.isBusinessHours()) {
        ...
        if (data && data.companies.length > 0) {
          ...
          if (supabaseSuccess) {
            console.log(
              `✨ Scraping concluído às ${brasiliaTime.toLocaleString("pt-BR", {
                timeZone: "America/Sao_Paulo",
              })}\n`
            );
          } else {
            console.log(
              "⚠️ Scraping concluído mas houve erros ao salvar no Supabase\n"
            );
          }
        } else {
  ...
  async getStats() {
    ...
    console.log(
      `\n📊 ESTATÍSTICAS - ${brasiliaTime.toLocaleString("pt-BR", {
        timeZone: "America/Sao_Paulo",
      })}`
    );
    console.log(
