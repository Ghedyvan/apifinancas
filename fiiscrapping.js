...
class InfoMoneyFIIScrapperService {
    constructor() {
        this.baseUrl = 'https://api.infomoney.com.br/ativos/top-alta-baixa-por-ativo/fii';
        this.params = {
            sector: 'Todos',
    ...
    try {
        console.log(`💾 Salvando ${fiiData.length} registros de FIIs no Supabase...`);
        
        // Verificar quantos registros já existem para hoje
        ...
        console.log(`📋 Registros existentes hoje: ${existingCount || 0}`);
        ...
        for (let i = 0; i < fiiData.length; i += batchSize) {
            ...
            if (error) {
                ...
                if (insertedCount > 0) {
                    console.log(`✅ Lote ${Math.floor(i/batchSize) + 1}: ${insertedCount} inseridos, ${skippedCount} atualizados/pulados`);
                } else {
                    console.log(`⚠️ Lote ${Math.floor(i/batchSize) + 1}: ${skippedCount} registros já existiam`);
                }
            }
        }

        console.log(`📊 Resumo Final: ${totalInserted} novos, ${totalSkipped} atualizados/existentes, ${errorCount} erros`);
        ...
        console.log(`📈 Total de registros hoje no banco: ${finalCount || 0}`);
    ...
    async getAllPages() {
        const brasiliaTime = this.getBrasiliaTime();
        console.log(`🏢 Iniciando coleta de dados de FIIs - ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})}`);
        ...
        try {
            ...
            if (!this.hasRunInitially || this.isBusinessHours()) {
                if (!this.hasRunInitially) {
                    console.log(`🎬 PRIMEIRA EXECUÇÃO FIIs - ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})}`);
                    this.hasRunInitially = true;
                } else {
                    console.log(`⏰ EXECUÇÃO PROGRAMADA FIIs - ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})}`);
                }
                ...
                if (data && data.fiis.length > 0) {
                    ...
                    if (supabaseSuccess) {
                        console.log(`✨ Scraping de FIIs concluído às ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})}\n`);
                    } else {
                        console.log('⚠️ Scraping de FIIs concluído mas houve erros ao salvar no Supabase\n');
                    }
                } else {
                    console.log('❌ Falha no scraping de FIIs\n');
                }
            } else {
                console.log(`⏸️  FIIs FORA DO HORÁRIO COMERCIAL - ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})} - Aguardando próxima execução...\n`);
            }
        } catch (error) {
    ...
    startScheduler() {
        ...
        console.log('⏰ Scheduler de FIIs ativo 24/7. Pressione Ctrl+C para parar.');
    }
    ...
    async getStats() {
        ...
        console.log(`\n🏢 ESTATÍSTICAS FIIs - ${brasiliaTime.toLocaleString('pt-BR', {timeZone: 'America/Sao_Paulo'})}`);
